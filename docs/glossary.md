...
- PCM（Pulse Code Modulation，脉冲编码调制）：按固定采样率测量声音振幅，再用固定位深把每次测量结果保存为数字样本。PCM 未经过有损压缩，数据量可由采样率、位深、声道数和时长直接计算。
- 采样率（sample rate）：每秒采集声音信号的次数，例如 48000 Hz 表示每秒产生 48000 个采样时刻；它不等于声音自身每秒振动的频率。
- 位深（bit depth）：每个样本用于表示振幅的有效位数，决定量化精度。16-bit 有 `2^16` 个量化级，24-bit 有 `2^24` 个量化级。
- 声道（channel）：每个采样时刻独立记录的音频信号路数。单声道记录一路，双声道分别记录左、右两路；声道增加不代表一定产生新的声音内容。
- PCM 码率公式：`采样率 × 位深 × 声道数`。PCM 数据大小为 `码率 × 时长 ÷ 8`；WAV 实际文件还包含容器头和元数据。
- 量化（quantization）：把连续的声音振幅映射到有限个数字刻度。位深越高，量化级越多、刻度越密，通常量化误差越小；量化精度描述振幅轴，不描述每秒采样次数。
- WAV：带有格式头的音频容器，可记录 PCM 的采样格式、采样率、声道数和位深，因此播放器能自动解释后续样本数据；文件大小还包含容器头开销。
- 裸 PCM：只有连续样本字节，没有容器头，无法自描述采样格式、采样率和声道数；播放时必须从外部提供这些参数。
- 小端序（little-endian）：低有效字节放在低地址或文件前面。`pcm_s16le` 的每个样本占 2 bytes，`pcm_s24le` 在文件中通常占 3 bytes。
- sample format（样本格式）：描述解码后样本在内存中的数值类型与布局。文件有效位深和内存容器宽度可能不同，例如 24-bit PCM 可由 FFmpeg 用 `s32` 的 32-bit 内存容器承载。

## Day 16

- AAC（Advanced Audio Coding）：有损音频编码。本次将 768 kbps PCM 压缩到约 127 kbps AAC 流，并封装在 M4A/MP4 容器中。
- Opus：支持语音和音乐、可变码率、丢包恢复与低延迟配置的有损音频编码；本次封装在 Ogg 容器中。
- 采样率 vs 编码码率：采样率是每秒对应的采样时刻数；编码码率是压缩数据每秒使用的 bit 数。压缩到更低码率不等于降低采样率。
- VBR 目标码率：编码器根据内容动态分配 bit 时的目标，不是严格上限。本次 `libopus -b:a 64k` 在默认 VBR 下实际约 96 kbps，`-vbr off` 后约 64.8 kbps。
- 编码帧填充：编码器常按固定数量的 samples 处理音频，末尾不足一帧时需要补齐。本次 AAC 解码后多 256 个 samples，与补齐到 1024-sample 帧边界一致。
- 有损解码不可逆：AAC/Opus 解码后会重新得到 PCM，但样本值只是听感接近原始信号，无法恢复原始文件的哈希。
